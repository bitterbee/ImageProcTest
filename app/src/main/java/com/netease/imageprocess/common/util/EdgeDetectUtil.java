package com.netease.imageprocess.common.util;
...
//        float k22 = 0.0f;
        ...
        for (int i = 0; i < w; i++) {
            for (int j = 0; j < h; j++) {
                // 获取卷积内核中各个元素对应像素的颜色值
                int[] p00 = mutiply(toGray(getARGB(pix, w, h, i - 1, j - 1)), k00);
                int[] p10 = mutiply(toGray(getARGB(pix, w, h, i, j - 1)), k10);
                int[] p20 = mutiply(toGray(getARGB(pix, w, h, i + 1, j - 1)), k20);
                int[] p01 = mutiply(toGray(getARGB(pix, w, h, i - 1, j)), k01);
                int[] p11 = mutiply(toGray(getARGB(pix, w, h, i, j)), k11);
                int[] p21 = mutiply(toGray(getARGB(pix, w, h, i + 1, j)), k21);
                int[] p02 = mutiply(toGray(getARGB(pix, w, h, i - 1, j + 1)), k02);
                int[] p12 = mutiply(toGray(getARGB(pix, w, h, i, j + 1)), k12);
                int[] p22 = mutiply(toGray(getARGB(pix, w, h, i + 1, j + 1)), k22);

                int[] pixARGB = add(p00, p10, p20, p01, p11, p21, p02, p12, p22);

                setColor(pix, w, h, i, j, uniform(pixARGB));
